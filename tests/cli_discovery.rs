...
special modules materializes `@area` and `@module` declarations from markdown heading annotations under the project root.
...
special modules does not require declarations to live in a privileged `_project/ARCHITECTURE.md` file.
*/
#[path = "support/cli.rs"]
mod support;
...
fn special_toml_ignore_patterns_hide_matching_paths_from_specs_modules_and_metrics() {
    ...
    let modules_output = run_special(&root, &["modules", "--metrics", "--verbose"]);
...
fn discovery_respects_gitignore_and_jjignore_by_default() {
    ...
    let modules_output = run_special(&root, &["modules"]);
    assert!(modules_output.status.success());
    let modules_stdout = String::from_utf8(modules_output.stdout).expect("stdout should be utf-8");
