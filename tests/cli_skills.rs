...
fn skills_install_live_state_skill() {
    let root = temp_repo_dir("special-cli-skills-live");
    let output = install_skills(&root);
    assert!(output.status.success());

    let skill = fs::read_to_string(root.join(".agents/skills/inspect-live-spec-state/SKILL.md"))
        .expect("live-state skill should exist");
    assert_eq!(skill, bundled_skill_markdown("inspect-live-spec-state"));
...
fn skills_install_planned_work_skill() {
    ...
    let skill = fs::read_to_string(root.join(".agents/skills/find-planned-work/SKILL.md"))
        .expect("planned-work skill should exist");
...
fn skills_bundle_reference_docs_for_progressive_disclosure() {
    ...
    let expected_reference_files = [
        ...
        "inspect-live-spec-state/references/state-walkthrough.md",
        "inspect-live-spec-state/references/trigger-evals.md",
