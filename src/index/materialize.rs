...
use crate::index::build_spec_metrics;
use crate::model::{
    DeclaredStateFilter, NodeKind, ParsedRepo, SpecDecl, SpecDocument, SpecFilter, SpecNode,
    VerifyRef,
};
...
pub(super) fn materialize_spec(
    ...
    let metrics = metrics.then(|| build_spec_metrics(root, &nodes));
    SpecDocument { nodes, metrics }
}
...
impl SpecFilter {
    fn matches(
        ...
        match self.state {
            ...
            DeclaredStateFilter::Current if planned => return false,
            DeclaredStateFilter::Planned if !planned => return false,
            DeclaredStateFilter::Current | DeclaredStateFilter::Planned => {}
        }
        if self.unverified_only
