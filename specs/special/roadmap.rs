/**
@spec SPECIAL.ROADMAP.SIGNED_ATTESTATIONS
special supports signed attestations.

@planned
*/

/**
@spec SPECIAL.ROADMAP.ATTESTATION_TTL_ENFORCEMENT
special enforces attestation review intervals in CI.

@planned
*/

/**
@spec SPECIAL.ROADMAP.CLAIM_HASH_BINDING
special binds attestations to spec revision hashes.

@planned
*/

/**
@spec SPECIAL.ROADMAP.SUPPORT_POLICY
special supports policy rules for which subtrees require verifies or attests.

@planned
*/

/**
@spec SPECIAL.ROADMAP.RICHER_FILTERING
special supports richer metadata and filtering views.

@planned
*/

/**
@spec SPECIAL.ROADMAP.SPEC_DIFFS
special supports diff views for spec graph changes.

@planned
*/

/**
@spec SPECIAL.ROADMAP.REPORT_UI_HARDENING
special hardens its report and HTML UI.

@planned
*/

/**
@spec SPECIAL.ROADMAP.PLANNED_RELEASE_WARNINGS
special warns when the current project version still contains planned specs targeted for that release or an earlier one.

@planned
*/
