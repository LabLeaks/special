...
11. Use `@module`, `@area`, and `@implements` for architecture intent instead of product specs. Reach for `special modules` when the question is structure, ownership, refactor shape, or whether code honestly implements a module rather than shipped behavior.
