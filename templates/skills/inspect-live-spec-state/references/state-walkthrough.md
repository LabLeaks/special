# State Walkthrough

Use this workflow when you need the current live state:

1. Run `special spec`.
2. Identify the exact claim or subtree you care about.
3. Use `special spec SPEC.ID` for a focused tree view.
4. Use `special spec SPEC.ID --verbose` when you need the actual support body.
5. Do not infer planned work from this view; it shows live claims only.

Example:

```sh
special spec EXPORT.CSV.HEADERS --verbose
```
